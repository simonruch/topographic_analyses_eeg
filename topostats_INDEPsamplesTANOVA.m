function [stat] = topostats_INDEPsamplesTANOVA(cfg,varargin)
% topostats_INDEPsamplesTANOVA performs topographic analysis of variance for
% independent samples: different conditions collected in different
...
%   MANDATORY OPTIONS:
%   cfg.ivar              = index of independent variable (conditions) in cfg.design
%
%   cfg.uvar              = index of unit-variable (subject-identifier)in cfg.design
%
...
%
%   stat.permutation.stats = gfp of topographic difference for all random samples
%
...
if numel(UNITS) < size(cfg.design,2)
    ...
    warning('Consider using DepsamplesTANOVA!');
end
...
stat            = [];
stat.cfg        = cfg;
stat.label      = {'topostats_INDEPsamplesTANOVA'};
%stat.input      = varargin;
%stat.data       = data;
stat.time       = cfg.time;
...
% Get unique conditons
CONDS   = unique(DESIGN(1,:)); %=> list of conditions

% COMPUTE AVERAGE MAPS FOR EACH CONDITION
CONDITION_AVERAGES = [];
for ci = 1:numel(CONDS)
